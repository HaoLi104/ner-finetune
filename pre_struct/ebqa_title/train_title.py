...
"""Alias/title EBQA 训练脚本：完全复用 EBQA 训练流程，仅问题模板改为别名格式。"""
...
def load_datasets(cfg: TrainConfig):
    ...
    if cfg.precomputed:
        ...
        ds_full = EnhancedQADataset(
            ...
            question_template=str(tb.get("question_template", "报告中用什么词表示'{key}'？")),
            # Fast tokenizer 在多线程下启用/切换截断配置会报错，禁用并发构建更稳
            use_concurrent_build=False,
            max_workers=None,
        ...
        ds_full = EnhancedQADataset(
            ...
            question_template=str(tb.get("question_template", "报告中用什么词表示'{key}'？")),
            # 见上方注释：禁用并发构建以避免 tokenizer 线程安全问题
            use_concurrent_build=False,
            max_workers=None,
...
def train_loop(cfg: TrainConfig):
    ...
    _save_history_json(history, cfg.output_dir, cfg.metrics_filename)
    print(
        f"[OK] Training finished. Artifacts in: {cfg.output_dir} "
        f"(best {'char-F1' if cfg.best_metric.lower()=='char_f1' else 'token-F1'}={best_key_metric:.4f})"
    )
...
if __name__ == "__main__":
    ...
    cfg_path = (
        os.environ.get("EBQA_TITLE_CONFIG_PATH")
        or os.environ.get("EBQA_CONFIG_PATH")
        or "pre_struct/ebqa_title/merged_config.json"
    )
